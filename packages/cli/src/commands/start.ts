...
/**
 * Start TUI dashboard inside a tmux session so Tab cycling works.
 *
 * If already inside tmux, spawns the TUI directly (inheriting the
...
function startTui(tuiDir: string, configPath: string | null): ChildProcess {
  ...
  if (isInsideTmux()) {
    // Already in tmux — run directly so switch-client works
  ...
  // Not in tmux — create a tmux session for the TUI, then attach
  const tuiEntrypoint = resolve(tuiDir, "dist/index.js");

  // Build env args for tmux new-session
  const envArgs: string[] = [];
  if (configPath) {
    envArgs.push("-e", `AO_CONFIG_PATH=${configPath}`);
  }

  // Kill stale TUI session if it exists
  try {
    execFileSync("tmux", ["kill-session", "-t", TUI_TMUX_SESSION], {
      timeout: 5_000,
      stdio: "ignore",
    });
  } catch {
  ...
  // Create a detached tmux session running the TUI
  execFileSync(
    "tmux",
    [
      "new-session",
